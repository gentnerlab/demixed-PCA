...
def dPCA(Y,ncomp,tolerance = 10e-3,maxloop=100):
    ...
    for o in range(len(marginals)):
        yo[o] = marg_average(Y,marginals[o]).real
    ...
    while True:
        ...
        Q,D = w_opt(einsum(Y,[0] + range(2,pdim+2),Ez,[1] + range(2,pdim+2),(0,1)),einsum(Ezz,range(2+pdim),range(2)),Q,D)
        W = dot(Q,D)
...
def QDloss(A,B,Q,D): 
    return trace(dot(A,dot(D,Q.T))) - 0.5*trace(dot(B,D**2))

def remean(X): 
    return X - einmean(X).reshape((X.shape[0],) + len(X.shape[1:])*(1,))

def einmean(X): 
    return einsum(X,range(len(X.shape)),(0,))/prod(X.shape[1:])    

def covmat(X): 
    return einsum(X,(0,)+tuple(range(1,len(X.shape))),X,(len(X.shape),)+tuple(range(1,len(X.shape))),(0,len(X.shape)))

def PCA(C,n): 
    return linalg.eig(C)[1][:,:n]
...
def invert_J(A,J): 
    return map(int,list(delete(arange(len(A.shape)),J)[1:]))
# J = list of indices
def marginalize(A,J): 
    return einsum(A,range(len(A.shape)),[0] + invert_J(A,J))/float(prod(array(A.shape)[J]))